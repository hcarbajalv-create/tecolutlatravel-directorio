...
> **ARCHIVADO (2026-08-11).** Este agente ya no está activo — se renombró con un punto al inicio (`.archivo-revisor-sitemap.md`) para que deje de cargarse como agente disponible. Las 17 verificaciones que hacía quedaron absorbidas dentro de `auditor-seo-tecolutla.md` (sección 9 "Indexabilidad", más partes de las secciones 3 y 8). Se conserva este archivo solo como referencia histórica — no editarlo ni volver a activarlo; si hace falta ajustar algo de lo que hacía, el cambio va en `auditor-seo-tecolutla.md`.
